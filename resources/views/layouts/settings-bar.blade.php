<div class="flex items-center space-x-4">
    <!-- Theme toggler -->
    <button class="flex items-center justify-center w-8 h-8 rounded-full bg-purple-500 text-white focus:outline-none focus:shadow-outline-purple"
    ...
    <div class="relative" x-data="{ open: false }">
        <button class="flex items-center justify-center w-8 h-8 rounded-full bg-purple-500 text-white focus:outline-none focus:shadow-outline-purple"
            @click="open = !open" @keydown.escape="open = false" aria-label="Account" aria-haspopup="true">
            <img class="object-cover w-4 h-4 rounded-full" src="{{ Auth::user()->profile_photo_url }}" alt="{{ Auth::user()->name }}"
                aria-hidden="true" />
        </button>

        <div x-show="open" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" @click.away="open = false" @keydown.escape="open = false"
